  ...
  return [
    ...new Set([
      ...
      ...Object.entries(aliases).flatMap(([latin, bangla]) =>
        n.includes(latin)
          ? [n.replaceAll(latin, bangla)]
          : n.includes(bangla)
            ? [n.replaceAll(bangla, latin)]
            : [],
      ),
    ]),
  ];
 ...
 */
function takeSpaced<T extends Candidate>(
  queue: T[],
  emitted: T[],
  now: number,
): T | undefined {
  ...
  for (const c of queue)
    remaining.set(c.author_id, (remaining.get(c.author_id) ?? 0) + 1);

  const previous = emitted.length
    ? emitted[emitted.length - 1].author_id
    : null;
  const eligible =
    previous === null ? queue : queue.filter((c) => c.author_id !== previous);
  ...
  for (const c of pool) {
    ...
    if (
      count > bestCount ||
      (count === bestCount && score(c, now) > score(best, now))
    )
      best = c;
  }
...
export function rank<T extends Candidate>(items: T[], now = Date.now()): T[] {
  const byScore = (a: T, b: T) =>
    score(b, now) - score(a, now) || a.id.localeCompare(b.id);
