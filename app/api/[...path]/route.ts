...
export async function GET(request: Request, context: Context) {
  try {
    ...
    if (path === "health") {
      ...
      return Response.json({ ok: true, sandbox: sandbox() });
    }
    if (path === "config") return Response.json({ sandbox: sandbox() });
    const u = await authenticate(request);
    const url = new URL(request.url);
    if (path === "data")
      return Response.json(await readApp(u, url), {
        headers: { "Cache-Control": "no-store" },
      });
    if (path === "live")
      return Response.json(await readLive(u, url), {
        headers: { "Cache-Control": "no-store" },
      });
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    need(path === "command", "Not found", 404);
    return Response.json(
      await command(await authenticate(request), JSON.parse(body)),
    );
  } catch (e) {
    return failure(e);
  }
