...
await tx(async (db) => {
  ...
  await db.query(
    "CREATE TABLE IF NOT EXISTS public.bdos_migration(name text PRIMARY KEY,checksum text NOT NULL,applied_at timestamptz DEFAULT now())",
  );
  for (const name of (await readdir("db/migrations"))
    ...
    const old = (
      await db.query(
        "SELECT checksum FROM public.bdos_migration WHERE name=$1",
        [name],
      )
    ).rows[0];
    if (old) {
    ...
    await db.query(sql);
    await db.query(
      "INSERT INTO public.bdos_migration(name,checksum) VALUES($1,$2)",
      [name, checksum],
    );
