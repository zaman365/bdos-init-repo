...
Sign in with any seeded number — in sandbox the OTP is returned by the API and
the form fills it in for you:

| Phone | Account | Roles |
|---|---|---|
| `+8801712345678` | nusrat | viewer + creator |
| `+8801912345678` | bogurashop | + seller |
| `+8801512345678` | operations | + admin |
| `+8801312345678` | shakib | + partner |
| `+8801812345678` | rifat | viewer + creator |
| `+8801612345678` | tamim | viewer + creator (a minor: LIVE and gifting refuse) |

**`APP_ORIGIN` must match the host and port you actually browse.** `lib/http.ts`
compares the request's `Origin` header to it with strict equality, so a
mismatch rejects every POST with `403 Request origin is not allowed` while GETs
keep working — which looks like a broken app rather than a config problem. If
port 3000 is taken, change both together:

```bash
npm run dev -- --port 3210      # and set APP_ORIGIN=http://localhost:3210
```

`.claude/launch.json` and `next-env.d.ts` are deliberately gitignored: the
first pins a port that is only free on one machine, and the second is
regenerated by Next and flips between `.next/types` and `.next/dev/types`
depending on whether `dev` or `build` ran last.
