...
const directory = () =>
  path.resolve(
    /* turbopackIgnore: true */ process.env.MEDIA_DIR ?? ".data/media",
  );
...
export async function upload(u: User, request: Request) {
  await flag(pool, "uploads");
  await rate(pool, `upload:${u.id}`, 10, 600);
  const limit = 22 * 1024 * 1024;
  need(
    Number(request.headers.get("content-length") ?? 0) <= limit,
    "Upload limit is 20 MB",
    413,
  );
  const reader = request.body?.getReader();
  need(reader, "Choose a file");
  ...
  while (true) {
    ...
    if (bytes > limit) {
      await reader.cancel();
      need(false, "Upload limit is 20 MB", 413);
    }
  ...
  need(file instanceof File, "Choose a file");
  need(
  ...
  const source = path.join(/* turbopackIgnore: true */ directory(), filename);
  await writeFile(source, bytesBuffer, { flag: "wx" });
  try {
    if (kind.mime.startsWith("video/")) {
      ...
      await unlink(source);
      filename = out;
      kind = { mime: "video/mp4", ext: "mp4" };
    }
    const size = (
      await stat(path.join(/* turbopackIgnore: true */ directory(), filename))
    ).size;
    await pool.query(
      "INSERT INTO app.media(id,owner_id,filename,mime,bytes) VALUES($1,$2,$3,$4,$5)",
      [id, u.id, filename, kind.mime, size],
    );
...
export async function mediaResponse(u: User, id: string, request: Request) {
  ...
  const m = await one(pool, "SELECT * FROM app.media WHERE id=$1", [id]);
  need(m, "Media not found", 404);
  if (m.owner_id !== u.id) {
    need(!(await blocked(pool, u.id, m.owner_id)), "Media not found", 404);
    need(
      await one(
        pool,
  ...
  const buffer = await readFile(
    path.join(/* turbopackIgnore: true */ directory(), m.filename),
  );
  ...
  if (range) {
    ...
    const end = match[1]
      ? Math.min(
          match[2] ? Number(match[2]) : buffer.length - 1,
          buffer.length - 1,
        )
      : buffer.length - 1;
